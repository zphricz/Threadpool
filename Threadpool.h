...
class Pool {
  public:
    ...
    void wait_for_all_jobs() {
        ...
        signal_main.wait(
            lk, [&] { return job_queue.empty() && running_threads == 0; });
    }
    ...
    template <typename F> void submit_task(F &&f) {
        {
            std::lock_guard<std::mutex> lk(m);
    ...
    template <typename F>
    std::future<typename std::result_of<F()>::type> submit_contract(F &&f) {
        ...
        while (true) {
            {
                ...
                signal_threads.wait(
                    lk, [&] { return !job_queue.empty() || !running; });
                if (!running) {
                    break;
                }
                running_threads++;
