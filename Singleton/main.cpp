  ...
  public:
    ...
    void test() {
      for (int i = 0; i < 100000; ++i) {
        submit_task(&Tester::run_once, this);
      }
    }
};
...
int main() {
...
#if 1
  {
    auto f1 = submit_contract(add_tester, 54, 12);
    cout << f1.get() << endl;
  }
#endif
#if 1
  {
    cout << "ACTUAL SWAP" << endl;
    {
      ...
      auto f = submit_contract(my_swap, std::ref(a), std::ref(b));
      f.get();
    ...
    cout << "BAD SWAP" << endl;
    {
      ...
      int b = 5;
      cout << "BEFORE: a = " << a << ", b = " << b << endl;
      auto f = submit_contract(my_bad_swap, a, b);
      f.get();
      cout << "AFTER: a = " << a << ", b = " << b << endl;
    }
  }
...
#define lim 1000
  ...
  for (int i = 1; i < lim; ++i) {
    futures.push_back(submit_contract(vec_func, i));
  }
  for (auto& f: futures) {
    auto v = f.get();
    for (auto& i: v) {
